...
const validateEmail = (email) => {
  const regex = /^[a-zA-Z]+[.]{0,1}[a-zA-Z]+@+[a-zA-Z]+[.]+[a-zA-Z]{3}\b/gm;
...
const validatePhoneNumber = (phoneNumber) => {
  const patter = /^(\(\d{3}\)|\d{3})[- ]?\d{3}[- ]?\d{4}$/;
  return patter.test(phoneNumber);
};
// //did this during class review.
...
const findTagNames = elements => {
  return elements.map(str => str.match(/<(\/[a-z]+\d?)>/g)).reduce((accu, currV)=> accu.concat(currV.map(element => element.slice(1,(element.length - 1)))), []);
};
//did this during class review.
/* ------------------------------------------------------------------------------------------------
TESTS
